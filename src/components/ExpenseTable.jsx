...
const ExpenseTable = () => {
  const { expenses, deleteExpense, isOverBudget, getOverspentAmount } = useBudget();
  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold text-gray-800">Expenses & Income</h2>
                ...
                <tr key={expense.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="py-3 px-2">
                    <span className={`text-sm font-medium ${
                      isOverBudget(expense.category) ? 'text-red-600' : 'text-gray-800'
                    }`}>
                      {expense.category}
                      {isOverBudget(expense.category) && (
                        <span className="ml-1 text-xs text-red-500">
                          (Over by ₹{getOverspentAmount(expense.category).toLocaleString()})
                        </span>
                      )}
                    </span>
                  </td>
